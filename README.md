<p align="center">
  <img width="250" height="250" src="https://raw.githubusercontent.com/Xseba360/muse/master/.github/logo.png">
</p>
...
When running a production instance, I recommend that you use the [latest release](https://github.com/Xseba360/muse/releases/).
...
```bash
docker run -it -v "$(pwd)/data":/data -e DISCORD_TOKEN='' -e SPOTIFY_CLIENT_ID='' -e SPOTIFY_CLIENT_SECRET='' -e YOUTUBE_API_KEY='' ghcr.io/xseba360/muse:latest
```
...
services:
  muse:
    image: ghcr.io/xseba360/muse:latest
    restart: always
    volumes:
      - ./muse:/data
    environment:
...
**Note**: if you're on Windows, you may need to manually set the ffmpeg path. See [#345](https://github.com/Xseba360/muse/issues/345) for details.
